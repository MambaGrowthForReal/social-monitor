"""Temporary FB run script - 5 keywords, GB+US, screenshots."""
...
TODAY = date.today()
MIN_DAYS, MAX_DAYS, MAX_ADS = 10, 30, 5
COUNTRIES = ['GB', 'US']
KEYWORDS  = ['salt trick weight loss', 'pink salt trick', 'natural mounjaro', 'belly fat', 'ice hack weight loss']
OUT_FILE  = os.path.join(os.path.dirname(os.path.abspath(__file__)), '..', 'data', '_fb_new.json')
...
def _is_english(text):
    markers = {'the','a','an','and','or','is','are','was','to','you','your','we','our',
               ...
               'now','my','of','in','it','i','have','not','do','lose'}
...
async def main():
    ...
    async with async_playwright() as pw:
        ...
        for country in COUNTRIES:
            ...
            for kw in KEYWORDS:
                ...
                print(f'  [{done:>2}/{total}] {kw:<35}', end=' ', flush=True)
                ads = await scrape_one(page, country, kw)
                print(f'-> {len(ads)} ads')
                all_ads.extend(ads)
                await asyncio.sleep(1.5)

        # dedup by URL or page+text
        seen, unique = set(), []
        for ad in all_ads:
        ...
        print(f'\n=== Screenshots ({len(unique)} unique ads) ===')
        for i, ad in enumerate(unique):
            print(f'  [{i+1:>2}/{len(unique)}] {ad["page"][:30]:<30}', end=' ', flush=True)
    ...
    print(f'\nSaved {len(unique)} ads -> {OUT_FILE}')
    print(f'With thumbnails: {sum(1 for a in unique if a.get("thumbnail"))}')
    for ad in unique:
        kb = len(ad.get("thumbnail","")) // 1024
        flag = {"US": "US", "GB": "GB"}.get(ad["pais"], "")
        print(f'  [{flag}] [{ad["dias_ativo"]}d] {ad["page"][:25]}: {ad["texto"][:55]}... [{kb}KB]')
